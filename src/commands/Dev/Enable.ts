...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        if (!key) return void (await M.reply(`Provide the command you want to enable Baka`))
        ...
        if (!(await this.client.DB.disabledcommands.findOne({ command: command.config.command })))
            return void M.reply(`${this.client.util.capitalize(command.config.command)} is already enabled Baka`)
        await this.client.DB.disabledcommands.deleteOne({ command: command.config.command })
        await M.reply(`*${this.client.util.capitalize(command.config.command)}* is now Enabled Enjoy yourselfe Baka`)
    }
}
